...
public class OwnerServiceImpl implements OwnerService {
	...
	private static final String OWNER_EMAIL_ID_NOT_FOUND = "owner.email.id.not.found";
	private static final String OWNER_EMAIL_ID_AND_MOBILE_NUMBER_NOT_FOUND = "owner.email.id.mobile.number.not.found";
	...
	@Override
	public OwnerDTO findOwner(String emailId, String mobileNumber) throws OwnerNotFoundException {
		Owner owner = ownerRepository.findOwner(emailId,mobileNumber);
		if (Objects.isNull(owner)) {
			throw new OwnerNotFoundException(String.format(PROPERTIES_CONFIG.getProperty(OWNER_EMAIL_ID_AND_MOBILE_NUMBER_NOT_FOUND), emailId,mobileNumber));
		}
		return MapperUtil.convertOwnerEntityToDto(owner);
	}
}
